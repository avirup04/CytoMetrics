...
class CytoMetricsApp:
    def __init__(self, root):
        ...
        tk.Button(root, text="🔄 Lasso Tool", command=self.run_lasso, bg="lightgreen").pack(pady=5)
    ...
    def run_lasso(self):
        # Check if an image is loaded before trying to open the tool
        if self.cell_img_path: 
            cyto_engine.launch_lasso(self.cell_img_path, self.final_ratio)
        else:
            messagebox.showwarning("Wait!", "Please upload a cell image first.")
